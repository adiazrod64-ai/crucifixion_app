    ...
    <div style={{ padding: "18px 18px 90px" }}>
      <div style={{
        ...
        <div style={{ fontFamily: "Cormorant Garamond", fontWeight: 700, fontSize: 32, marginTop: 6, lineHeight: 1.1 }}>
          {account?.name ? `Welcome, ${account.name.split(" ")[0]}` : "Crucifixion"}
        </div>
      ...
      {account?.type === "ministry" && (
        <button onClick={() => goto("give")} style={{
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontFamily: "Source Serif 4", fontWeight: 600, fontSize: 14, color: T.ink }}>Set up {account.ministryName || "your"} church page</div>
...
function OnboardingScreen({ onComplete }) {
  const [mode, setMode] = useState(null); // null | 'individual' | 'ministry'
  const [form, setForm] = useState({ name: "", email: "", ministryName: "", city: "" });
  const [error, setError] = useState("");

  const submit = () => {
    if (!form.name.trim() || !form.email.trim()) { setError("Please fill in your name and email."); return; }
    if (mode === "ministry" && !form.ministryName.trim()) { setError("Please add your ministry or parish name."); return; }
    onComplete({
      type: mode,
      name: form.name.trim(),
      email: form.email.trim(),
      ministryName: mode === "ministry" ? form.ministryName.trim() : null,
      city: form.city.trim(),
      createdAt: now(),
    });
  };
      ...
      <div style={{ padding: "28px 22px", flex: 1 }}>
        {!mode && (
          <>
            ...
            <button onClick={() => { setMode("individual"); setError(""); }} style={{
            ...
            <button onClick={() => { setMode("ministry"); setError(""); }} style={{
              width: "100%", textAlign: "left", background: T.card, border: "1px solid #EADFC0", borderRadius: 16,
              padding: 18, cursor: "pointer", display: "flex", gap: 14, alignItems: "center",
            }}>
              <div style={{ width: 44, height: 44, borderRadius: 999, background: "#F4E9CB", display: "flex", alignItems: "center", justifyContent: "center", border: `1px solid ${T.gold}55`, flexShrink: 0 }}>
                <Landmark size={20} color={T.oxblood} />
              </div>
        ...
        {mode && (
          <>
            <button onClick={() => { setMode(null); setError(""); }} style={{ background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 6, color: T.navy, fontFamily: "Inter", fontSize: 13, fontWeight: 600, marginBottom: 14 }}>
              <ArrowLeft size={16} /> Back
            </button>

            <div style={{ fontFamily: "Cormorant Garamond", fontWeight: 700, fontSize: 24, color: T.navy }}>
              {mode === "individual" ? "Create your account" : "Create your ministry account"}
            </div>
            <div style={{ fontFamily: "Inter", fontSize: 12.5, color: T.mist, marginTop: 4 }}>
              {mode === "individual" ? "Just a couple details to get started." : "You'll be able to set up your church page and donation options right after."}
            </div>
            ...
            <div style={{ marginTop: 18, display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <div>
                <FieldLabel icon={<User size={13} />}>{mode === "ministry" ? "Contact name" : "Your name"}</FieldLabel>
            ...
            <div style={{ marginTop: 18 }}>
              <IconBtn icon={<CheckCircle2 size={15} />} label={mode === "individual" ? "Create account" : "Create ministry account"} onClick={submit} />
            </div>
          </>
        )}
      </div>
...
export default function App() {
  ...
  useEffect(() => {
    const t = setTimeout(async () => {
      const acc = await loadPersonal("account", null);
      setAccount(acc);
      setPhase(acc ? "app" : "onboarding");
    }, 1600);
    return () => clearTimeout(t);
  }, []);

  const completeOnboarding = useCallback(async (acc) => {
    await savePersonal("account", acc);
    setAccount(acc);
    setPhase("app");
  }, []);
  ...
  if (phase === "onboarding") return <OnboardingScreen onComplete={completeOnboarding} />;
